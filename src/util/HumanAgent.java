...
public class HumanAgent extends Agent{
    ...
    public HumanAgent(int serialNum,char type,Vertex initial,Vertex goal, ArrayList<Action> actions) {
    ...
    private void choose(Graph g) throws IOException {
        ...
        Edge e = null;
        switch(cmd){
            case 'v': System.out.println(g.toString());
                choose(g);
                break;
            case 's': System.out.println(super.location.toString());
                System.out.println("\n--------------------------\n");
                choose(g);
                break;

            case 'r': if (!askEsc && super.location.getMUnits() > 0){
                askEsc = true;
            ...
            case 'p': if (!reqChems && super.location.getChemicals() > 0){
                reqChems = true;
            ...
            case 'd': System.out.println("~~~ where would you like to go?:");
                e = parseNum(g);
                super.actions.get(1).action(this, e,reqChems,askEsc,true);
                break;
            case 'n': super.actions.get(0).action(this, null,false,false,true);
                break;
        }// end case
    }// end function choose
